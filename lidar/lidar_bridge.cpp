    ...
    while (true) {
        ...
        if (n > 0) {
            ...
            if (cmd == "RESTART") {
                ...
                syslog(LOG_INFO, "Received RESTART command via port %d", CMD_PORT);
                if (g_reader) {
...
int main() {
    openlog("lidar_bridge", LOG_PID | LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "Starting lidar_bridge process");

    // 1. Connect to Python first
    std::cout << "[Bridge] Connecting to Python APF..." << std::endl;
    int sock = connect_to_python();
    if (sock < 0) {
        std::cerr << "[Bridge] Could not connect. Start python3 lidar_apf.py first."
                  << std::endl;
        return 1;
    }
        ...
        if (ret == 102) {
            ...
            if (lreader->getPointCloud(cloud)) {
                ...
                for (const auto& pt : cloud.points) {
                    ...
                    if (!send_point(sock, pt.x, pt.y, pt.z)) {
                        syslog(LOG_ERR, "Python APF client disconnected. Exiting parse loop.");
                        std::cerr << "[Bridge] Python disconnected." << std::endl;
                        close(sock);
            ...
            if (elapsed >= 5.0) {
                syslog(LOG_INFO, "Parse loop active: frames=%llu, pts_sent=%llu", (unsigned long long)frames, (unsigned long long)pts_sent);
    ...
    syslog(LOG_INFO, "Exiting application loop, commanding stopLidarRotation()");
    lreader->stopLidarRotation();
    close(sock);
    closelog();
    return 0;
